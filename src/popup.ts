...
interface ShowImagePreviewParams {
  imgUrl: string;
  color?: string[];
  onClose?: () => void;
  svgIcon?: string;
...
interface ShowBottomPopupParams {
  title?: string;
  content?: string;
...
class Popup {
  ...
  eventListeners: Array<{ element: HTMLElement, callback: EventListener }> = [];
  mask: HTMLDivElement;
  contentBox: HTMLDivElement;
  queue: Array<() => void> = []; // 弹窗队列
  isShowing: boolean = false; // 当前是否正在显示弹窗
  ...
  // 中间有弹框的 
  middleBox(param: MiddleBoxParams): void {
  ...
  alert(param: AlertParams): void {
    ...
    if (btns.length > 0) {
      btns.map((item, index) => {
        const btnItem = this.createButton(item, btnStyles?.[index] || defaultBtnStyle);
  ...
  showImagePreview({ imgUrl, color, onClose, svgIcon }: ShowImagePreviewParams): void {
    ...
    const svgString = svgIcon || `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M6 6L18 18M18 6L6 18" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>`;
    const parser = new DOMParser();
  ...
  msg(str: string, param?: MsgParams): void {
  ...
  load = (paramObj: LoadParams = {}): void => {
    /**
     *  @param paramObj load 参数;
     *  @param paramObj.width loading SVG 宽度;
     *  @param paramObj.height loading SVG 高度;
     *  @param paramObj.color loading SVG 颜色;
  ...
  addClickListeners(elements: Array<Element | HTMLElement>, callback: EventListener): void {
    elements.forEach(element => {
      element.addEventListener('click', callback);
      this.eventListeners.push({ element: element as HTMLElement, callback });
    });
  }
